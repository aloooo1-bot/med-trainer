...
ORDERABLE CATEGORIZATION RULE: availableImaging MUST contain only radiological studies (X-ray, CT, MRI, PET), ultrasound, nuclear medicine, and endoscopic-imaging procedures. The following MUST NOT appear in availableImaging: Peripheral Blood Smear (PBS), Thoracentesis, Lumbar Puncture, Bone Marrow Biopsy, Paracentesis, Pericardiocentesis, Arthrocentesis, or any invasive procedure or lab morphology study. PBS belongs in availableLabs. Procedural fluid analyses (pleural fluid from Thoracentesis, CSF from LP, synovial fluid) belong in availableLabs with results in labResults as a fluid-analysis panel. ECG belongs in availableImaging as "ECG" or "ECG (12-lead)" with its result as a text report in imagingResults — NOT as a lab panel. Miscategorization teaches students that PBS is an imaging study — a fundamental clinical-reasoning error.
ABNORMAL LAB RESOLUTION RULE: Every lab component flagged as "high", "low", or "critical" in labResults MUST be addressed in either (a) teachingPoints — naming its clinical significance in this case, OR (b) differentialExplanations — explaining why it distinguishes this diagnosis. An abnormal result with no educational resolution creates a confounding artifact. If the abnormal result is incidental (e.g., mild thrombocytopenia in sepsis), teachingPoints MUST include a brief explanatory note: "The [result] is [expected/incidental] in this context because [mechanism]."
LAB STATUS ARITHMETIC RULE: The status field for every lab component MUST arithmetically match the value vs. referenceRange. If referenceRange is "80–100" and value is 79, status MUST be "low". If referenceRange is "< 2" and value is 6, status MUST be "high". If a result is expected to be abnormal for therapeutic/diagnostic reasons (e.g., elevated HbF in HbSS is protective), the referenceRange field MUST include a case-specific explanatory note (e.g., "< 2% in unaffected adults; elevated HbF is protective in HbSS and NOT clinically concerning in this context — do not interpret as pathological").
HIDDEN FINDING DISCOVERABILITY RULE: Every clinically significant item in hiddenHistory.hiddenSymptoms or hiddenHistory.fullHistory that changes diagnosis, staging, or management MUST be paired with at least one keyQuestion that would elicit it on direct questioning. A hidden finding with no keyQuestion trigger provides zero educational value. If you would grade a student for missing this finding, it MUST have a keyQuestion. This strengthens KEY-QUESTION HIDDEN-ANSWER COVERAGE: all classification-changing hidden items must have a paired trigger question.
DIAGNOSIS-ACUITY CONSISTENCY RULE: The correctDiagnosis label MUST match the clinical acuity in HPI and vitals. If the label contains "Crisis," "Acute Severe," "Emergent," or similar acuity descriptors, the HPI MUST include clear acute-severity features (rapid onset, functional impairment, hemodynamic instability) and vitals MUST reflect the stated acuity. If HPI and vitals show near-normal or chronic presentation, the label MUST NOT claim "Crisis" or "Severe." Mismatch between label and clinical picture teaches incorrect severity classification.
PATIENT-ANCHORED TEACHING RULE: Teaching points giving dosing or management recommendations MUST account for this specific patient's characteristics: (a) if the patient is already on the recommended medication, acknowledge this rather than recommending initiation as if new; (b) if standard dosing is age-adjusted (e.g., low-dose levothyroxine initiation in elderly patients), name the age-adjusted recommendation for this patient's age; (c) if a quoted severity criterion does NOT apply to this patient's demographics, state the patient's actual score explicitly rather than citing the threshold as if this patient meets it.
CASE-SPECIFIC CRITERIA ACCURACY RULE: When teachingPoints quote a validated scoring system (Ranson criteria, CURB-65, Wells score, Rockall score — NOT "Rockford score", CTSI/Balthazar grade, Rai staging, etc.), the explanation MUST include: (a) which score this specific patient achieves, and (b) what that score means for this patient's risk/management tier. When CT Severity Index or Balthazar grade appears in imagingResults, teachingPoints MUST explain the scoring methodology.
TEACHING WORKUP AVAILABILITY RULE: Any test referenced in teachingPoints or differentialExplanations as "diagnostic," "confirmatory," or "used to distinguish" MUST appear in availableLabs or availableImaging so the student can order it. If a test is essential for the differential but unorderable, the case contradicts itself. Exception: tests framed as "follow-up at X weeks" or "outpatient" may be mentioned as teaching context without being acutely orderable.
MENINGITIS FEVER RULE: Bacterial meningitis cases MUST present with temp ≥ 101.5°F (38.6°C) unless the case explicitly models a partially-treated or immunocompromised presentation (in which case the HPI MUST explain blunted fever). A bacterial meningitis case with only low-grade fever (≤ 100.4°F) without atypical-host explanation undermines the core teaching objective of the classic fever–nuchal rigidity–altered mental status triad.
LIGHT'S CRITERIA ACCURACY RULE: For empyema, parapneumonic effusion, or exudative pleural effusion cases, teachingPoints MUST state Light's criteria correctly: pleural fluid is an exudate if ANY ONE of — (a) pleural LDH > 2/3 × upper limit of normal serum LDH, (b) pleural protein/serum protein ratio > 0.5, (c) pleural LDH/serum LDH ratio > 0.6. Do NOT state the LDH threshold as a fixed absolute value without anchoring it to the serum upper limit — absolute cutoffs without this reference are clinically inaccurate.
ACUTE MI LIPID TIMING RULE: For ACS/acute MI cases, lipid panels drawn within 24 hours of presentation are unreliable (acute-phase reactants suppress LDL 10–20%). If a lipid panel appears in labResults for an ACS case: (a) the referenceRange field MUST include "Acute-phase values unreliable — repeat fasting panel 4–6 weeks post-event," and (b) teachingPoints MUST NOT use acute lipid values as the basis for statin decision-making. Statin indication in ACS is driven by the ACS event itself, not the acute lipid level.
PHYSICAL EXAM SENSITIVITY CONTEXT RULE: Teaching points quoting sensitivity or specificity for physical exam maneuvers (Lachman test, anterior drawer, McMurray's, FABER, Phalen's, Thompson test, etc.) MUST include an examiner-experience qualifier: "in experienced hands" or "examiner-dependent." Published sensitivities are derived from specialist examiners; without this caveat, students are misled about real-world reliability of these maneuvers.
TEACHING–DATA NUMERIC FIDELITY RULE: teachingPoints, differentialExplanations, and the *Findings narrative fields MUST NOT mischaracterize the case's own quantitative data. If a lab or vital is flagged abnormal (status "high"/"low"/"critical") in labResults or vitals, prose MUST NOT describe that same parameter as normal, absent, or within range — e.g., do NOT write "without leukocytosis" or "a normal WBC" when the case's WBC is flagged high; do NOT write "afebrile" when temp is elevated. Before asserting any value is normal/absent/within-range in prose, verify it against the actual value and status in the case data. When the teaching intent is that a finding CAN be normal/unimpressive in this condition, anchor the statement to the patient's actual value (e.g., "the WBC is only mildly elevated at 11.8, illustrating that a robust leukocytosis is not required early in alcoholic pancreatitis") rather than falsely calling it normal.
HEMATURIA RBC MORPHOLOGY RULE: When a case includes hematuria, the RBC morphology described in urineFindings / urinalysis MUST match the bleeding source. Glomerular bleeding (glomerulonephritis — IgA nephropathy, PSGN, lupus nephritis, anti-GBM, etc.) produces DYSMORPHIC RBCs (acanthocytes/echinocytes) ± RBC casts. Urologic / post-renal bleeding (ureteral or kidney stone, urothelial trauma, cystitis, BPH, urinary tract malignancy) produces ISOMORPHIC (eumorphic) normal-shaped RBCs WITHOUT RBC casts. NEVER describe RBCs as "dysmorphic" in a stone or other urologic/post-renal case, and NEVER pair a stated "urinary tract / obstructive / non-glomerular source" conclusion with "dysmorphic" RBC morphology — the morphology and the stated source must agree.
DIFFERENTIAL REASONING DATA RULE: Populate differentialPriors, testImpacts, and mechanism so the trainer can drive a live differential board. (1) differentialPriors MUST contain exactly one entry for the correct diagnosis PLUS one entry for every item in differentials — names must match those strings verbatim. Priors are pre-test weights (before ANY test is ordered) reflecting how the presentation alone would rank the hypotheses; the correct diagnosis need NOT have the highest prior (at Clinical/Advanced a red herring may lead early). (2) testImpacts MUST include an entry for every test in expectedLabs and expectedImaging (others optional), and within each test an effect for EVERY name in differentialPriors. The effect describes how THIS case's actual resulted value moves that hypothesis: "confirms" (pathognomonic/definitive for it), "supports" (makes it more likely), "neutral" (no change), "argues-against" (less likely), "excludes" (effectively rules it out). (3) Consistency: the case's pathognomonic/confirmatory test MUST have effect "confirms" for the correct diagnosis, and the test that rules out each can't-miss differential MUST have effect "excludes" for that differential. After ordering the expected workup, the correct diagnosis must end up the most likely. (4) mechanism is 2-3 sentences of pathophysiology connecting the key findings to the disease process — accurate, not hand-wavy.`
...
export const JSON_SCHEMA_TEMPLATE = `{
  ...
  "differentialPriors": [{ "name": "<the correct diagnosis OR an exact differentials entry>", "prior": <pre-test weight 0.05-0.9 before any tests are ordered>, "category": "<leading|alternative|cant-miss>" }, "...one entry for the correct diagnosis AND one for EVERY differentials entry. See DIFFERENTIAL REASONING DATA RULE"],
  "testImpacts": {
    "<exact test name from availableLabs or availableImaging>": {
      "<name from differentialPriors>": { "effect": "<confirms|supports|neutral|argues-against|excludes>", "why": "<short reason tied to THIS case's actual result for this test>" }
    }
  },
