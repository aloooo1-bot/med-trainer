...
MODERN STANDARD-OF-CARE RULE: expectedLabs and expectedImaging (the must-order workup the student is graded against) MUST contain only tests that are part of the CURRENT standard of care for this presentation. Do NOT list obsolete or superseded studies as expected/core — e.g. plain skull radiographs for head trauma (CT head is standard), plain abdominal films where CT is indicated, or IVP where CT urography is standard. An outdated test may still appear in availableImaging/availableLabs as an orderable distractor, but it must NEVER be an expectedLabs/expectedImaging must-order. When in doubt, list the study a current emergency physician or hospitalist would actually order today.`
