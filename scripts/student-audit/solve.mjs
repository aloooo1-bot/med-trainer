...
ORDERABLE CATEGORIZATION RULE: availableImaging MUST contain only radiological studies (X-ray, CT, MRI, PET), ultrasound, nuclear medicine, and endoscopic-imaging. The following MUST NOT appear in availableImaging: PBS, Thoracentesis, Lumbar Puncture, Bone Marrow Biopsy, Paracentesis, Pericardiocentesis, Arthrocentesis, or any invasive procedure. PBS belongs in availableLabs. ECG belongs in availableImaging as "ECG (12-lead)" with a text report in imagingResults. Procedural fluid results (pleural fluid, CSF, synovial fluid) belong in availableLabs.
ABNORMAL LAB RESOLUTION RULE: Every lab component flagged as "high", "low", or "critical" MUST be addressed in teachingPoints or differentialExplanations. An abnormal result with no educational resolution creates a confounding artifact. If the result is incidental, note: "The [result] is [expected/incidental] because [mechanism]."
LAB STATUS ARITHMETIC RULE: The status field MUST arithmetically match value vs. referenceRange. If referenceRange is "80–100" and value is 79, status MUST be "low". For therapeutically-expected abnormals (e.g., elevated HbF in HbSS), add a case-specific note in referenceRange explaining this.
HIDDEN FINDING DISCOVERABILITY RULE: Every clinically significant hidden finding that changes diagnosis, staging, or management MUST be paired with at least one keyQuestion that would elicit it. A hidden finding with no keyQuestion trigger provides zero educational value.
DIAGNOSIS-ACUITY CONSISTENCY RULE: The correctDiagnosis label MUST match the clinical acuity in HPI and vitals. If the label says "Crisis" or "Severe," HPI and vitals must reflect that acuity. Near-normal presentation must not carry an "Acute Severe" or "Crisis" label.
